...
function App() {
  ...
  const fetchNewsAPI = async () => {
    try {
      ...
      const response = await fetch("https://newsapi.org/v2/everything?q=keyword&apiKey=d579601285ee44509f16fa5311447cee");
      ...
      <div className='grid grid-cols-1 lg:grid-cols-3 gap-10'>
        {
          newsLists.map((news) => (
            <div key={news.title} className='ring-1 ring-inset pb-5 ring-gray-300 rounded-lg'>
              <div>
                {news.urlToImage ? (
                  <img src={news.urlToImage} alt={news.url} className='w-full rounded-t-lg bg-fixed' />
                ) : (
